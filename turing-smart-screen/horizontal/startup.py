...
COM_PORT = "AUTO"
WIDTH, HEIGHT = 320, 480  # native portrait dims -- library rotates via SetOrientation, don't swap these
REVISION = "A"
FONT_PATH = "res/fonts/jetbrains-mono/JetBrainsMono-ExtraBold.ttf"
    ...
    while True:
        try:
            ...
            while True:
                ...
                text = (
                    f"{now.center(56)}\n"
                    f"{'AMD BC-250'.center(56)}\n"
                    f"\n"
                    f"{f'{cu_count}CU  {core_count}C/{thread_count}T'.center(28)}{'Metrics'.center(28)}\n"
                    f"{'RDNA2:':<6}{int(gpu_load):>3}% {int(gpu_clock):>5} {'MHz':<4}{int(gpu_temp):>3} {'°C':<3}{'APU Power:':<14}{ppt_watts:>6} {'W':<4}\n"
                    f"{'Zen2:':<6}{int(cpu_load):>3}% {int(cpu_clock):>5} {'MHz':<4}{int(cpu_temp):>3} {'°C':<3}{'APU mV:':<16}{apu_voltage_mV:>4} {'mV':<4}\n"
                    f"{'':<28}{'CPU mV:':<16}{cpu_voltage_mV:>4} {'mV':<4}\n"
                    f"{'16GB GDDR6'.center(28)}{'APU Fan:':<16}{fan_rpm:>4} {'RPM':<4}\n"
                    f"{'VRAM:':<7}{vram_used:>5.1f} {'GB':<15}{'NVMe Temp:':<16}{nvme_temp:>4} {'°C':<4}\n"
                    f"{'RAM:':<7}{ram_used:>5.1f} {'GB':<15}{'Disk Read:':<16}{fmt_rate(disk_read, 5)} {'MB/s↓':<8}\n"
                    f"{'':<28}{'Disk Write:':<16}{fmt_rate(disk_write, 5)} {'MB/s↑':<8}\n"
                    f"{'':<28}{'Net Mbps:':<13}{fmt_rate(rx_speed, 4)} {'↓':<3}{fmt_rate(tx_speed, 4)} {'↑':<2}"
                )
